...
# $(call defstruct, struct_name, $(call defslot, slotname, value), ...)
define defstruct
$(eval all_structs += $1)										\
...
$(strip															\
	...
	$(eval all_instances :=										\
		$(filter-out $($(strip $1)),$(all_instances))			\
	)															\
	$(foreach v, $($(strip $1)_def_slotnames),					\
		$(eval $(instance)_$v := )								\
	)															\
...
$(if $(filter $2,$($(call struct-name,$1)_def_slotnames)),,		\
	$(error Instance '$($(strip $1))' 							\
			does not have slot '$(strip $2)')					\
)
endef
...
{																			\
	...
	$(foreach s,$($(strip $1)_def_slotnames),								\
  		$(strip																\
...
{															\
	$(foreach s, $($(strip $1)_def_slotnames),				\
		$(strip { "$s": "$($(strip $1)_def_$s_default)" })	\
	)														\
}
endef
...
{															\
	...
	$(foreach s,$($(tmp_name)_def_slotnames),				\
		$(strip												\
...
{															\
	$(foreach s,$($(call struct-name,$1)_def_slotnames),	\
		"$(strip $(call get-value,$1,$s))"					\
	)														\
}
endef
